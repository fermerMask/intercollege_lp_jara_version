...
var ctx = document.getElementById("chart_chuo_tech");
var myRadarChart = new Chart(ctx, {
  // !!以下のtypeは共通の為触れないでください
  type: "radar",
  data: {
    //変更項目　　　データの各項目のラベルを設定してください(上から時計回り)
    labels: ["練習後のグダグダ", "新歓の必死さ", "enjoy", "伸びしろ", "声"],
    datasets: [
      {
...
var ctx = document.getElementById("chuo_tech");
var myRadarChart = new Chart(ctx, {
  // !!以下のtypeは共通の為触れないでください
  type: "radar",
  data: {
    //変更項目　　　データの各項目のラベルを設定してください(上から時計回り)
    labels: ["レース中の声量", "女子率", "仲の良さ", "エルゴ", "理系率"],
    datasets: [
      {
        //変更項目　上記のlabelsで設定した項目の数値を設定できます(上から時計回り)
        data: [5, 2, 5, 2, 5],
